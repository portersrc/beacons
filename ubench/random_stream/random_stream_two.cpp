...
void init_random_array1() {
  ...
  for (int i = 0; i < loop_bound1; i++) {
    for (int j = 0; j < loop_bound1; j++) {
      for (int k = 0; k < loop_bound1; k++) {
        random_array1[((counter++) * 64) % MAX_SIZE] = rand() % 100; 
      }
    }
  }
}
...
void process_array1() {
  for (int i = 0; i < loop_bound2; i++) {
    for (int j = 0; j < loop_bound2; j++) {
      for (int k = 0; k < loop_bound2; k++) {
        // This will ensure that this will be all cache hits.
        sum += random_array1[k % 10];
      }
    }
  }
}
...
void init_random_array2() {
  ...
  for (int i = 0; i < loop_bound1; i++) {
    for (int j = 0; j < loop_bound1; j++) {
      for (int k = 0; k < loop_bound1; k++) {
        random_array2[((counter++) * 64) % MAX_SIZE] = rand() % 100; 
      }
    }
  }
}
